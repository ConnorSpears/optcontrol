...
from GP_functions6 import *
from open_loop6 import *
from LQOCP6 import *
from closed_loop6 import *



def RefineSamples(surrogate,dsurrogate,X1, samples,num_refinements,error):
  ...
  for i in range(num_refinements):
    ...
    m,s,obj,solerr,poly,u,ud,x1d,x2d = RefineModel(surrogate,X1, samples,error)
  ...
  μ2, Σ2 = GP(X1, y1, X2, error, exponentiated_quadratic)

  #Function to evaluate the GP mean at a point
  def mu(x):
    X_new = np.array([[x]])  # New x-value for prediction
    μ_new,Σ_new = GP(X1, y1, X_new, error, exponentiated_quadratic)
    return μ_new
...
def RefineModel(surrogate,X1, samples,error) -> float:
