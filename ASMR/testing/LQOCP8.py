...
def LQOCP(sample,x1,x2,x1d,x2d,σ2,poly,polyderi):
  ...
  def rule1(model, t):
    ...
    B = 0.8 * x2[eval] + 1.5 * ( x1[eval]*pyo.sin(11*x1[eval]-2) + 0.05 )
    
    A = 0.8 * polyderi(x1[eval])*x1d[eval]*x2[eval] + 1.5* ( polyderi(x1[eval])*x1d[eval]*(x1[eval]*pyo.sin(11*x1[eval]-2)+0.05) + poly(x1[eval])*((pyo.sin(11*x1[eval]-2)) + 11*x1[eval]*cos(11*x1[eval]-2)) )
