...
def SolveModel(func):
  ...
  #   return model.x1d[t] == pyo.cos(model.u[t])+ 0.8 * func(model.x1[t])*model.x2[t] + 1.5*func(model.x1[t])*(model.x1[t]*pyo.sin(11*model.x1[t])+0.05)
  ...
  model.constraints.add(expr = model.x2[0]==0.1)
  model.constraints.add(expr = model.x2[1]<=0.1)
  model.constraints.add(expr = model.x2[1]>=0.1)
