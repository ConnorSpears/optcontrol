...
def RefineSamples(surrogate,X1, samples,num_refinements):
  ...
  for i, sensitivity_functions in enumerate(all_obj):
    ...
    if i < num_refinements:
      ...
      ax[i * 3 + 1].scatter(sample_x, sample_y, marker='.')
...
def RefineModel(surrogate,x, samples) -> float:
  ...
  return sol, solution_error_estimate[sol], [modelActual.x1[:](),modelActual.x2[:](),modelApprox.x1[:](),modelApprox.x2[:]()], solution_error_estimate, poly
