...
from GP_functions7 import *
from open_loop7 import *
from LQOCP7 import *
from closed_loop7 import *
...
def RefineSamples(surrogate,dsurrogate,X1, samples,num_refinements):
  ...
  for i in range(num_refinements):
    ...
    m,s,obj,solerr,poly,u,ud,x1d,x2d,hyperparams = RefineModel(surrogate,X1, samples,hyperparams)
  ...
  hyperparams_list = optimize_hyperparameters(X1, y1, exponentiated_quadratic, [hyperparams['l'],hyperparams['σp']])
  ...
  def mu(x):
    ...
    μ_new,Σ_new = GP(X1, y1, X_new, exponentiated_quadratic,hyperparams)
    return μ_new
...
def RefineModel(surrogate,X1, samples,hyperparams) -> float:
  ...
  return sol, solution_error_estimate[sol], [modelActual.x1[:](),modelActual.x2[:](),modelApprox.x1[:](),modelApprox.x2[:]()], solution_error_estimate,poly(x_values),u,ud,modelActual.x1d[:](),modelActual.x2d[:](),hyperparams
