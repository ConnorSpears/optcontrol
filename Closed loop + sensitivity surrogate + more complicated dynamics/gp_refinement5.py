...
from GP_functions5 import *
from open_loop5 import *
from LQOCP5 import *
from closed_loop5 import *
...
def RefineSamples(surrogate,dsurrogate,X1, samplerange,num_refinements):
  ...
  for i in range(num_refinements):
    ...
    optimal_sample, samples, poly_fit_to_samples, obj,solerr,poly,u,ud,x1d,x2d = RefineModel(surrogate,X1, samplerange)
...
def RefineModel(surrogate,X1, samplerange) -> float:
  ...
  for sample in samples:
    ...
    sensitivity_functions.append(model_out.s1[:]())
  ...
  # #EGO
  # errbound = 0.0000001 * 0.99**(len(X1)-1)
  ...
  #   #if vals_in_bounds is empty, we need to add a random point
  #   if len(vals_in_bounds) == 0:
  #     vals_in_bounds.append(np.random.uniform(0,1))
  ...
  # print("EGO converged in", count, "steps")
  # print("------------------------------------")
  ...
  return optimal_sample, samples, p, [modelActual.x1[:](),modelActual.x2[:](),modelApprox.x1[:](),modelApprox.x2[:]()], solution_error_estimate,polyout,u,ud,modelActual.x1d[:](),modelActual.x2d[:]()
