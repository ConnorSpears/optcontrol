...
def diff_riccati(t, P_flat, A_func, B_func, Q, R, x1ref, x2ref, x1dref, uref, udref, actual, dactual,dt,N):
    ...
    A = A_func(uref[i], x1ref[i], x2ref[i], x1dref[i], actual, dactual)
...
def A_func(u,x1, x2, x1_dot, actual, dactual):
    return np.array([[dactual(x1) * x1_dot * x2, actual(x1)], [1/2 * (u-x2)+2*(1-x1), -1/2 * x1]])

def B_func(u, u_dot, x1):
    return np.array([[-np.sin(u) * u_dot], [np.cos(u) * u_dot + 1/2*x1]])
