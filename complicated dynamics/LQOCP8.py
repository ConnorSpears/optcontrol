...
def LQOCP(sample,x1,x2,x1d,x2d,σ2,poly,polyderi):
  ...
  def rule1(model, t):
    ...
    A1 = polyderi(x1[eval])*x1d[eval]*(0.4*pyo.sin(3*x2[eval])+x2[eval])
    A2 = poly(x1[eval]) *(1.2*pyo.cos(3*x2[eval])*x2d[eval]+0.5)
    B = 0.4*pyo.cos(3*x2[eval])+0.5*x2[eval]
  ...
  def rule2(model, t):
    ...
    A1 = 2*(1-x1[eval])*x1d[eval] + x1d[eval]*poly(x1[eval])+x1[eval]*polyderi(x1[eval])*x1d[eval]
    A2 = 0
    B = x1[eval]

    return model.s2d[t] == A1*model.s1[t] + A2*model.s2[t] + B*model.delta[t]
