...
def SolveModel(func):
  ...
  def _ode_rule1(model, t):
    return model.x1d[t] == pyo.cos(model.u[t]) + func(model.x1[t])*model.x2[t] + func(model.x1[t])*(0.6*pyo.sin(3*model.x2[t]) + 0.5*model.x2[t])

  #dynamics
  def _ode_rule2(model, t):
    return model.x2d[t] ==  pyo.sin(model.u[t]) - (1-model.x1[t])**2 + model.x1[t]*func(model.x1[t]) 
  ...
  model.constraints.add(expr = model.x2[0]==0.05)
  model.constraints.add(expr = model.x2[1]<=0.2)
  model.constraints.add(expr = model.x2[1]>=-0.2)
  ...
  for i in model.t:
    ...
    model.constraints.add(expr = model.x2[i] <= 0.4)
